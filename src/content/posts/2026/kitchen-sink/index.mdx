...
import screenshotLight from './screenshot-light-chrome.png';
import screenshotDark from './screenshot-dark-chrome.png';
import fixtureVector from './fixture-vector.svg';
...
Every example below is followed by the source that produces it. Copy the
snippet, not the rendered result — and note the language tag on each: `mdx`
means the file has to be `.mdx`. The Markdown tier next door shows what the same
idea looks like when it cannot be.

A component has to be imported before it is used, once per file, under the
frontmatter:
...
The `../../../../` climbs out of the post's own bundle — four levels, because a
Post sits at `<year>/<slug>/` inside the collection. Images are imported too
rather than referenced as strings; that is what gets them a `srcset`, a width
and a height.
...
`kind` is required and has no default. A figure that cannot say what it is has
no business being framed.
...
<ArticleImage
  ...
  kind="photo"
  breakout
  caption="What the reader should notice about it."
/>
```

`breakout` is the layout axis and `caption` is the component tier. Neither is
available to a plain Markdown image — that is the two-tier limit, and this is
...
<ImagePair
  ...
  caption="A pair is one figure with one caption, not two figures side by side."
/>
...
<ImagePair
  kind="photo"
  images={[
    { src: imageA, alt: 'What the first one shows' },
    { src: imageB, alt: 'What the second one shows' },
  ]}
  caption="One caption for the pair, because it is one figure."
/>
```

### Screenshots, in both chromes

The kind that answers every framing question the opposite way from `photo`:
never cropped, never dimmed, and given an edge by the page. Both captures below
have outer pixels that are exactly the site's own ground colours — light and
dark — which is the case the kind exists for. Whichever theme you are reading
in, one of them is the "wrong" one, and neither should read as a hole in the
page.

<ArticleImage
  src={screenshotLight}
  alt="Screenshot of an application with light chrome: a title bar, a sidebar with nine items, and rows of content"
  kind="screenshot"
  caption="Light chrome. On the light theme its outer pixels match the page exactly."
/>

<ArticleImage
  src={screenshotDark}
  alt="Screenshot of the same application with dark chrome"
  kind="screenshot"
  caption="Dark chrome. Same problem, the other way round."
/>

```mdx
<ArticleImage
  src={myScreenshot}
  alt="Screenshot of the settings pane, with the sync toggle switched on"
  kind="screenshot"
  caption="What the reader should notice in it."
/>
```

Same component, one word different — and every framing decision inverts. This is
the only way to get the plate: the Markdown tier cannot tell a screenshot from a
photograph, because the path is its only hook and a filename is too fragile to
carry the distinction.

### A chart, which is drawn rather than photographed

The kind a Research Visual arrives as. Framed and otherwise left alone — no
dimming on a dark ground, because dimming a drawing is treating it as a
photograph and a research visual's whole job is to be read accurately. It is the
one kind whose caption usually carries an attribution, which is why the aside
below it takes `sourceHref` and `sourceLabel` rather than folding the credit
into the caption text.

<ArticleImage
  src={fixtureVector}
  alt="Fixture vector chart: four bars of increasing height above a baseline"
  kind="chart"
  caption="A chart states what it measures. This one measures nothing — it is a fixture."
/>

```mdx
<ArticleImage
  src={myChart}
  alt="Interval chart showing the 2025 estimates, two of which cross no change"
  kind="chart"
  caption="My simplified reading of the source's estimates."
/>
```

Alt text describes the image; it is never the attribution. The source goes in
the caption, visibly — see **Research Visual** in `CONTEXT.md`.
...
<MediaAside
  ...
  placement="right"
  caption="What the reader should notice about it."
  sourceHref="https://example.com/the-source"
  sourceLabel="Data: Example"
>
  The text that sits beside the figure. Two or three paragraphs read as a pair
  with the media; one paragraph looks dwarfed by it.
</MediaAside>
```

`placement` is the third axis — where it sits — and it is deliberately not part
of `kind`. Moving a figure to the other side never changes what it is.
...
```mdx
<YouTube id="dQw4w9WgXcQ" title="What the clip is" poster={myPoster} />
```

The `id` is the part of the URL after `watch?v=`. `title` is required — it names
the frame for a screen reader, and "YouTube video player" names nothing.
...
```mdx
<Video src="my_clip.mp4" poster={myPoster} />

<Video src="my_clip.mp4" />
```

`src` is an R2 key, not a local import — video is the one medium that does not
travel in the bundle. `poster` is optional and omitting it is a real choice: no
poster gives the browser's first frame, which beats a mismatched image.
...
## Images without a component

A plain Markdown image needs no import and works identically in `.md` — it is
the Markdown tier, sitting inside an `.mdx` file. It gets framing keyed on its
path, and never a figure or a caption:

```md
![What the image shows](./my-image.png)
```

A diagram is a fenced block tagged `mermaid`, written exactly as the two above
are written in this file's source. It is not shown as a snippet here for a
mechanical reason worth knowing: a fenced block cannot contain a fenced block.
