...
function sourcePosts(base: string): Array<{ slug: string; frontmatter: string }> {
  const posts: Array<{ slug: string; frontmatter: string }> = [];
...
export function futureScheduledSlugs(base = 'src/content/posts', now: Date = new Date()): string[] {
  const slugs: string[] = [];
  for (const { slug, frontmatter } of sourcePosts(base)) {
...
/** Draft or not-yet-live post slugs that must never appear in a sitemap. */
export function unpublishedSlugs(base = 'src/content/posts', now: Date = new Date()): string[] {
  const slugs: string[] = [];
  for (const { slug, frontmatter } of sourcePosts(base)) {
    ...
    const match = frontmatter.match(/^\s*liveFrom\s*:\s*["']?([0-9T:-]+)["']?/m);
    if (match && !liveFromHasPassed(match[1], now)) slugs.push(slug);
  }
  return slugs;
}
