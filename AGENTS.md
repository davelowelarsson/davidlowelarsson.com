...
- **Post-push checks**: monitor GitHub Actions jobs/workflows to a terminal
  result and address failures. Do not wait for, poll, or inspect the Cloudflare
  Preview Deployment in routine work. Only verify the deployed preview when the
  task is specifically about deployment behavior, or David asks for the URL or
  a deployed visual check.
